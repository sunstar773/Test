...
public class ImageCodeServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
      ...
      /* req.getSession().setAttribute("checkcode", randomString);*/
        String session = DistributedSession.getSession(req, resp);
        //将生成的验证码的，放到redis中
        RedisUtil.setEx(KeyUtil.codeKey(session),randomString,5*60);
       /* String sessionId = DistributedSession.getSessionId(req, resp);
        // 将生成的验证码的值，放到redis中
        RedisUtil.setEx(KeyUtil.buildCodeKey(sessionId), randomString, SystemConst.CODE_EXPIRE);
*/
